...
import { buildProviderLaunchArgs, hostResident } from "@brooswit/drovr";
...
import { residentLabelFor, residentLaunchFrom, withoutForcedPermissionMode, withoutPermissionMode } from "../../src/spawn/herdr";
import { drovrClientOverCli } from "../../src/spawn/herdr-cli-client";
...
describe("drovr's residents", () => {
  ...
  test("a fork is started by bakr's own host, which drovr's request cannot express", async () => {
    ...
    expect(r.ok).toBe(true);
    // Hosted as drovr hosts a resident (BAKR-52 moves forks onto drovr itself), never in a `bakr ` workspace.
    expect(host.panes[0]!.label).toBe("drovr bakr-a");
    expect(host.panes[0]!.name).toBe("bakr-a");
    expect(host.starts()).toEqual([["--resume", "s1", "--fork-session"]]);
  });

  test("a fork is listed and stopped by drovr like any resident", async () => {
    const host = makeFakeHost({ mintSessionId: () => "forked" });
    const r = await launch(DIR, ["--resume", "s1", "--fork-session"], { runCommand: host.runCommand, label: "@a", ...instant() });
    if (!r.ok) throw new Error(r.error);
    expect((await listBackgroundSessions({ runCommand: host.runCommand })).map((s) => s.id)).toEqual([r.id]);
    expect(await stopSession(r.id, { runCommand: host.runCommand })).toEqual({ ok: true });
    expect(host.panes).toEqual([]);
  });

  test("a fork is refused while a live pane holds the agent's name, as drovr refuses a second resident", async () => {
    const host = makeFakeHost();
    const first = await launch(DIR, [], { runCommand: host.runCommand, label: "@a", ...instant() });
    if (!first.ok) throw new Error(first.error);
    const r = await launch(DIR, ["--resume", "s1", "--fork-session"], { runCommand: host.runCommand, label: "@a", ...instant() });
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error).toContain(`label-taken: pane ${first.id}`);
    expect(host.panes).toHaveLength(1);
  });

  test("no launch of any kind — fresh, resume or fork — creates a pane in a `bakr ` workspace", async () => {
    const host = makeFakeHost();
    const launches: [string, string[]][] = [["@f", []], ["@r", ["--resume", "s1"]], ["@k", ["--resume", "s2", "--fork-session"]]];
    for (const [label, args] of launches) {
      const r = await launch(DIR, args, { runCommand: host.runCommand, label, ...instant() });
      if (!r.ok) throw new Error(r.error);
    }
    const created = host.calls.filter((c) => c[1] === "workspace" && c[2] === "create").map((c) => c[c.indexOf("--label") + 1]);
    // FALSIFIER: bakr's own `bakr <id>` label on any of these is a launch through the legacy path, which is list-and-stop only.
    expect(created).toEqual(["drovr bakr-f", "drovr bakr-r", "drovr bakr-k"]);
  });
...
describe("the permission mode drovr adds: stripped, and the strip asserts (BAKR-35's decision; goes with BAKR-51)", () => {
  ...
  test("the pair present: exactly drovr's --permission-mode bypassPermissions is removed, nothing else", () => {
  ...
  // FALSIFIER for the two below: a strip that passes an argv through when it does not match starts claude in whatever mode drovr chose.
  test.each([
    ["the pair absent", ["--session-id", "s"], /no longer carries --permission-mode bypassPermissions/],
    ["the pair spelled another way", ["--permission-mode=bypassPermissions"], /no longer carries --permission-mode bypassPermissions/],
    ["a different mode instead", ["--permission-mode", "acceptEdits"], /no longer carries --permission-mode bypassPermissions/],
    ["a different mode as well", ["--permission-mode", "bypassPermissions", "--permission-mode", "plan"], /another permission flag, --permission-mode/],
    ["a skip-permissions flag as well", ["--permission-mode", "bypassPermissions", "--dangerously-skip-permissions"], /another permission flag, --dangerously-skip-permissions/],
  ] as const)("%s: refused with an error", (_label, args, why) => {
    expect(() => withoutPermissionMode(args)).toThrow(why);
  });

  test("a drovr that started claude in another mode: hostResident fails loudly (`start-failed`), nothing starts, and its workspace is closed", async () => {
    const host = makeFakeHost();
    const stripped = withoutForcedPermissionMode(drovrClientOverCli(host.runCommand));
    // Stands in for a drovr bump: whatever mode hostResident asks for arrives as `plan` instead.
    const bumped = { ...stripped, agent: { ...stripped.agent, start: (p: Parameters<typeof stripped.agent.start>[0]) => {
      const args = [...(p.args ?? [])];
      args[args.indexOf("bypassPermissions")] = "plan";
      return stripped.agent.start({ ...p, args });
    } } } as typeof stripped;
    const r = await hostResident(bumped, { provider: "claude", cwd: DIR, label: "bakr-a" }, { mintSessionId: () => "s", ...instant(), wait: instant().sleep });
    expect(r.ok).toBe(false);
    if (!r.ok) {
      expect(r.reason).toBe("start-failed");
      expect(r.detail).toContain("no longer carries --permission-mode bypassPermissions");
    }
